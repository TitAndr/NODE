...
const createCar = async (data) => {
  ...
  const userCarsCacheKey = `user_cars:${newCar.userId}`;
  const cars = await Car.find({ userId: newCar.userId }).populate(
    "userId",
  );
  await client.set(userCarsCacheKey, JSON.stringify(cars), {
    EX: 10800,
  });
  return newCar;
};
...
const getUserCars = async (userId) => {
  const cacheKey = `user_cars:${userId}`;
  const cachedCars = await client.get(cacheKey);
  if (cachedCars) {
    return JSON.parse(cachedCars);
  }
  const cars = await Car.find({ userId }).populate("userId");
  await client.set(cacheKey, JSON.stringify(cars), { EX: 10800 });
  return cars;
};
